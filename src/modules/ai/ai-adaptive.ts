...
/**
 * Returns a prompt string to append to the system prompt for Pro/Elite users.
 * Pass stats from the DB (same stats object used for dashboard).
 */
export function getAdaptiveBehaviorContext(stats: UserStats): string {
  ...
  const profile: BehaviorProfile = { depth, trajectory, weakDim, streak: stats.streak };
