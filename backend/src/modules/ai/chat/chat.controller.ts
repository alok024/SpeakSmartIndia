 ...
 * off to chat.service.ts. Injects adaptive behavior context (Pro/Elite)
 * and structured cache keys into every AI call.
 *
 * Prompt assembly lives in prompt/prompt.service.ts — this file only
...
// Per-call-type response caps: a hint is naturally 1-3 sentences while
// structured feedback (scores + corrections + model answer) needs more
// room. An explicit req.body.max_tokens from the client always wins —
// this table only governs the default when none is given.
...
/**
 * Resolves the max_tokens budget for this call: explicit client value if
