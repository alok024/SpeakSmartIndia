...
import { getUserProfile, saveOnboarding as saveOnboardingForUser } from './profile.service';
import { getWeakAreasForUser } from '../../analytics/reports/weak-areas.service';
...
export const getMe = asyncHandler(async (req: Request, res: Response) => {
  ...
  ok(res, {
    user: {
      ...
      plan:  dbUser.plan,
      name:  dbUser.name || '',
      // Required by ProtectedRoute's onboarding gate and admin gate — must be
      // present on every /me response or those client-side guards will
      // permanently redirect authenticated users.
