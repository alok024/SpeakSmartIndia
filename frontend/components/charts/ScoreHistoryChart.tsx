...
interface RechartsPayloadItem extends TooltipPayloadItem { index?: number }
...
function ScoreTooltip({ active, payload, prevScores = [] }: CustomTooltipProps) {
  ...
  const item   = (payload[0] as RechartsPayloadItem);
  const score  = item.payload.score;
  const rawIdx: number = item.index ?? prevScores.length - 1; // safe fallback: last entry
  const prev  = rawIdx > 0 ? prevScores[rawIdx - 1] : null;
  const delta = prev != null ? +(score - prev).toFixed(1) : null;
  const date  = item.payload.created_at
    ? new Date(item.payload.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })
    : '';
  return (
    <div style={{
      ...
      {item.payload.profession && (
        <div style={{ fontSize: 11, color: 'var(--text-2)', marginTop: 3 }}>{item.payload.profession}</div>
      )}
      {delta != null && (
        <div style={{
